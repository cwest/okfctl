...
tag-hygiene reports two or more `tags` spellings that fold to one canonical form (case, trimmed, a single trailing 's', and — for tags — separator-insensitive so run-book, run_book and runbook are one value). Like type-hygiene, it never rejects a value: it only reports spellings that fold together, so a curator can pick one. It never flags two genuinely distinct tags.
