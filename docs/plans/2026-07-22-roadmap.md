...
## Increment 1—Walking Skeleton—shipped (#2)
...
## Increment 2—Reserved-file lifecycle + remaining node verbs—shipped (#3, #4)
...
## Increment 3—`lint` (the curation differentiator), structural checks—shipped (#5)
...
## Increment 4—`graph` export + `serve` web visualizer—shipped (#6)

**Delivers:** `graph export` (json/dot; SVG via `dot -Tsvg` pipe) and `serve` (embedded `net/http` +
`go:embed` interactive graph). §13.4 front-end approach **decided: vanilla JS +
`go:embed`** ([ADR 0006](../adr/0006-vanilla-js-embedded-visualizer.md)). Depends on: increment 1.

## Increment 5—`okfctl-search` semantic plugin + vector index—shipped (#8, #9, #10, #11; core lexical search #23)

**Delivers:** the PATH-dispatch `okfctl-search` plugin (§8): `--semantic`,
`index build` (content-hash-keyed vector store recording `model`+`dim`),
`related`. Adopts the `cwest/knowledge-base` `Embedder` protocol (name/dim/encode)—MUST NOT reinvent it. **§13.2 fork decided: native-Go re-implementation**—a
pure-Go Model2Vec + WordPiece embedder, no Python
([ADR 0005](../adr/0005-pure-go-embedder.md)). The vector store is a flat
Go-native JSON file, not `sqlite-vec` ([ADR 0004](../adr/0004-flat-json-vector-store.md)).
Then wire `lint`'s semantic findings (similar-but-unlinked, orphan-no-neighbors).
Depends on: increments 1 + 3.

## Increment 6—Type templates + `validate --templates` overlay—shipped (#7)
...
## Increment 7—Registry / connect + plugin install convenience—shipped (#21, #22)
