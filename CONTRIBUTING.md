...
`okfctl` is a **consumer of a specification it does not own**. The
...
4. Run the conformance gate below and confirm it is green.
...
# 3. Real corpus — the layer fixtures cannot substitute for. Point the built
...
Layer 3 is not optional: a hand-built fixture only holds shapes the author
...
negative control is the one that gets skipped, and it is the load-bearing one.
...
- **Gate output.** Pin before/after counts, not "it passes." `validate 3→0,
  broken-link 0→0` is a result; a green checkmark is not. A count that did *not*
  move is as load-bearing as one that did — it is the control proving the change
