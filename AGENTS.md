...
**This trap is live and it has already caught us.** The obvious-looking source—`openknowledge-sh/openknowledge`, `Wiki/SPEC.md`—is a *pinned mirror*, and it is
...
- **No CGO, no Python, no ONNX.** Pure Go; `CGO_ENABLED=0` must build. (This
  is a rule about the *shipped binary*. CI *tooling* may use other runtimes—the site build uses Node, and the plugin-manifest gate uses Python's
  `check-jsonschema` to validate against the pinned schema URL.)
- **Prose uses CLOSED-UP Chicago em dashes** (`word—word`), never the spaced
  form (`word — word`). This is the house style for all reader-facing prose:
  `README.md`, `CONTRIBUTING.md`, `docs/**`, the site's Markdown and rendered
  copy, and this file. The rule is prose-only: em dashes inside fenced code
  blocks, inline `code` spans, CLI help-text output, and generated reference
  files follow their own medium's typography, not prose style—leave them, and
  where CLI help is the SOURCE of a generated doc (`docs/commands/README.md`),
  leave the source alone and let the generated file match it. The prose gate
  (`site/scripts/scan_page_prose.py`) enforces this on the reader-facing Markdown
  surfaces and fails on a spaced em dash in a prose block, while staying silent
  inside code and on generated docs.
