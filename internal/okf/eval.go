...
type EvalOptions struct {
	...
	//
	// Calibrated against the real cwest/knowledge-base corpus (239 nodes). The
	// legitimate epistemic vocabulary bottoms out at "draft"/"active" (2 nodes
	// each) and authority at "SYNTHESIS" (2 nodes); the genuine drift lives at
	// count 1 ("authority: high", one stray lowercase "verified"). A floor of 2
	// therefore isolates exactly the count-1 outliers without false-positiving a
	// small-but-real grade. See docs/specs/2026-08-07-taca-eval.md.
	GradeVocabularyFloor int
}

// defaultGradeVocabularyFloor: a grade value carried by only ONE node in the
// corpus is treated as drift, not vocabulary. Measured against the real 239-node
// cwest/knowledge-base by sweeping the floor: at 1 the check can never fire (no
// value has <1 carrier); at 2 it flags exactly the two genuine count-1 outliers
// ("authority: high" typo, "authority: DEPRECATED" lifecycle-into-trust drift);
// at 3 it false-positives legitimate count-2 grades (SYNTHESIS, epistemic draft,
// epistemic active). So 2 isolates real drift without punishing small-but-real
// grades. See docs/specs/2026-08-07-taca-eval.md.
