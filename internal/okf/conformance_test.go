...
//
//   - §6:  index files carry no frontmatter (bundle-root exception: okf_version
//          only).
//   - §7:  log.md is a date-grouped list, newest-first, ISO-8601 date headings.
//   - §11: okf_version marker semantics on the bundle-root index.
//
...
// isoDateHeading matches an OKF §7 date heading: a `## ` line whose text is an
// ISO-8601 YYYY-MM-DD date.
var isoDateHeading = regexp.MustCompile(`^## \d{4}-\d{2}-\d{2}$`)

// assertLogConformsSection7 checks a log.md body against the OKF §7 grammar:
...
func assertLogConformsSection7(t *testing.T, body string) {
	t.Helper()
	lines := strings.Split(strings.TrimRight(body, "\n"), "\n")
	if len(lines) == 0 || !strings.HasPrefix(lines[0], "# ") {
		t.Fatalf("§7: log.md must open with a `# ` title heading; got:\n%s", body)
	}
	var dates []string
	for _, line := range lines[1:] {
		if strings.HasPrefix(line, "## ") {
			if !isoDateHeading.MatchString(line) {
				t.Errorf("§7: date heading must be `## YYYY-MM-DD`; got %q in:\n%s", line, body)
			}
...
func TestConformance_ScaffoldValidatesClean(t *testing.T) {
	...
	// §6: the scaffolded index carries no frontmatter.
	raw, err := os.ReadFile(filepath.Join(dir, "index.md"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.HasPrefix(string(raw), "---\n") {
		t.Errorf("§6: scaffolded index.md must have no frontmatter; got:\n%s", raw)
	}

	// §7: the scaffolded log is well-formed.
	logBody, err := ReadLog(dir)
	if err != nil {
		t.Fatal(err)
	}
	assertLogConformsSection7(t, logBody)
}
...
func TestConformance_GeneratedBundleValidatesClean(t *testing.T) {
	...
	// §6: the generated index carries no frontmatter (no okf_version marker in
	// this bundle — no on-disk key, no .okf sidecar... but Scaffold writes a
	// .okf, so the §11 carve-out applies: assert exactly okf_version if present).
	assertGeneratedIndexSection6And11(t, dir)

	// §6 body grammar / no unsanctioned boilerplate.
	idxRaw, err := os.ReadFile(filepath.Join(dir, "index.md"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(idxRaw), "Generated by") {
		t.Errorf("§6: generated index must not carry `Generated by` boilerplate; got:\n%s", idxRaw)
	}
	if strings.Contains(string(idxRaw), "type: Index") {
		t.Errorf("§6: generated index must never emit `type: Index`; got:\n%s", idxRaw)
	}

	// §7: the log after an append is well-formed and newest-first.
	logBody, err := ReadLog(dir)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(logBody, "Added wine and lifting neighborhoods.") {
		t.Errorf("§7: appended entry missing from log:\n%s", logBody)
	}
}

// assertGeneratedIndexSection6And11 checks the on-disk generated bundle-root
// index against §6/§11: it either carries no frontmatter, or carries a
// frontmatter block whose sole key is okf_version.
func assertGeneratedIndexSection6And11(t *testing.T, dir string) {
	t.Helper()
	raw, err := os.ReadFile(filepath.Join(dir, "index.md"))
	if err != nil {
		t.Fatal(err)
	...
	if !has {
		return // §6: no frontmatter — conformant.
	}
	var keys []string
	...
	if len(keys) != 1 || keys[0] != "okf_version" {
		t.Errorf("§11: generated root index frontmatter must contain exactly okf_version; got keys %v in:\n%s", keys, raw)
	}
}

func TestConformance_GeneratedIndexPreservesOkfVersionMarker(t *testing.T) {
	// §11 marker semantics: a bundle whose root index declares okf_version must
	...
	if !strings.Contains(string(raw), `okf_version: "0.1"`) {
		t.Errorf("§11: index build must preserve the okf_version marker; got:\n%s", raw)
	}
	assertGeneratedIndexSection6And11(t, dir)
	...
	if f := Validate(b2); len(f) != 0 {
		t.Errorf("§11-marker bundle must validate clean after index build; got findings: %v", f)
	}
}
