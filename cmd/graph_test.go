...
func TestGraphExport_SVGFormatNamesGraphvizPipe(t *testing.T) {
	dir := writeGraphFixture(t)
	_, err := runOKF(t, "graph", "export", dir, "--format", "svg")
	if err == nil {
		t.Fatalf("--format svg should exit non-zero (no native svg format)")
	}
	msg := err.Error()
	if !strings.Contains(msg, "dot -Tsvg") {
		t.Fatalf("svg error should name the Graphviz pipe (dot -Tsvg), got: %q", msg)
	}
}
